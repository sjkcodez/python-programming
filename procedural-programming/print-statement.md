...
Short for Pretty Print, this is a specialized built-in tool.   
If you try to print a massive dictionary or a long nested list with the standard print(), it will display as one, unreadable line. pprint breaks the data down into indented blocks automatically.
...
If you are running Python inside an interactive terminal session (like a Jupyter Notebook), you don't even need a function. Typing any raw variable name or expression and pressing Enter will automatically force Python to display its value on the screen
